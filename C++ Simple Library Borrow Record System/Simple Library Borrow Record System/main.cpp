...
struct BooksRec { // 15 books
	...
	float bookPrice;
	Status bookStatus;
	std::time_t bookBorrowedDate;
	std::time_t bookReturnedDate;
	std::string bookTitle;
	std::string bookAuthor;
	BooksRec() : bookID(0), bookDuration(0), bookPrice(0.0f), bookStatus(Status::AVAILABLE), bookBorrowedDate(std::time(nullptr)),
		bookReturnedDate(std::time(nullptr)), bookTitle(""), bookAuthor("") { }
};
...
void borrowBook(const int& studentID, const int* bookID, const int& countBorrowBooks, float& totalPrice) {
	int j = 0;
	for (int i = 0; i <= studentCount; i++) {
		if (students[i].studentID == studentID) {
	...
	for (int i = 0; i < bookSize; i++) {
		if (books[i].bookID == bookID[k]) {
			...
			books[i].bookReturnedDate += (static_cast<std::time_t>(books[i].bookDuration) * secondsInDay);
		...
		if (students[i].studentID == studentID) {
			for (int j = 0; j < bookSize; j++) {
				if (books[j].bookID == bookID) {
					...
					books[j].bookReturnedDate = std::time(nullptr);
		...
		if (students[i].studentID == studentID) {
			...
			for (int j = 0; j < bookSize; j++) {
				if (books[j].bookTitle == students[i].booksBorrowed[k]) {
					...
					std::cout << "Book Title: " << std::setw(30) << books[j].bookTitle << '\n';
					std::cout << "Book Author: " << std::setw(30) << books[j].bookAuthor << '\n';
					std::cout << "Book Price: " << std::setw(30) << books[j].bookPrice << '\n';
					std::cout << "Book Duration: " << std::setw(20) << books[j].bookDuration  << " day(s)" << '\n';
					std::cout << "Book Borrowed Date: " << std::setw(30) << std::ctime(&books[j].bookBorrowedDate);
					std::cout << "Book Returned Date: " << std::setw(30) << std::ctime(&books[j].bookReturnedDate);
					std::cout << "Book Status: " << std::setw(30);
				...
				else if (k == students[i].numberOfBorrowedBooks) {
					return;
				}
			}
			break;
...
void Initialise() {
	...
	for (int i = 0; i < bookSize; i++) {
		...
		books[i].bookBorrowedDate = std::time(nullptr);
		books[i].bookReturnedDate = std::time(nullptr);
...
bool checkIfStudentIDisValid(const int& studentID, const bool& ifViewRecord, const bool& ifReturnBook)  {
	if (ifViewRecord || ifReturnBook) {
		for (int i = 0; i <= studentCount; i++) {
			if (students[i].studentID == studentID) {
				return true;
			}
		...
		system("cls");
		{
			...
			switch (ch) {
				...
				case '2':
					...
					std::cout << "Enter student ID: ";
					std::cin >> studentID;
					if (checkIfStudentIDisValid(studentID, false, false)) {
						system("cls");
							...
							if (checkIfBookIDisValid(bookID)) {
								...
								if (borrowAgain == 'Y' || borrowAgain == 'y') {
									...
									if (borrowOrNot == 'Y' || borrowOrNot == 'y') {
										borrowBook(studentID, tmpBookID, countBorrowBooks, totalPrice);
						...
						if (countBorrowBooks == MAX_TO_BORROW || (numberOfBorrowedBooks + countBorrowBooks) == MAX_TO_BORROW) {
							...
							std::cin >> borrowOrNot;
							if (borrowOrNot == 'Y' || borrowOrNot == 'y') {
								borrowBook(studentID, tmpBookID, countBorrowBooks - 1, totalPrice);
								displayBorrowedBooks(tmpBookID, studentID, true, numberOfBorrowedBooks);
								payment(totalPrice);
							}
						}
				...
				case '3':
					...
					std::cin >> studentID;
					system("cls");
					if (checkIfStudentIDisValid(studentID, false, true)) {
				...
				case '4':
					...
					std::cin >> studentID;
					system("cls");
					if(checkIfStudentIDisValid(studentID, true, false)) {
						viewRecord(studentID);
					}
					else {
						std::cout << "Invalid student ID!\n";
